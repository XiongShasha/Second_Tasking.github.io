...
var inputsArr=loadAllItems();
 var Com_inputs=[{}];
 var res=[];
 for(let i=0;i<inputs.length;)//计算inputs数组内重复的元素以及重复的个数
  {
   var count =0;
  ...
  for(let j=i+1;j<inputs.length;j++)//将重复的元素删除
      if(inputs[j]=inputs[i])
        for(let k=j+1;k<inputs.length;k++)
          inputs[k-1]=inputs[k];
  for(let i=0;i<inputs.length;i++) //构建出一个新的包含所有商品信息的数组
  {
    for(let j=0;j<inputsArr.length;j++)
      if(inputs[i]==inputsArr[j].barcode)
      {
...
{
  var per_Sum=[];
  for(let i=0;i<Com_inputs.length;i++)
  {
...
var tol_Sum=t_Sum(per_Sum);
  var str='';
  for(var i=0;i<inputs.length;i++)
  {
     str+='名称：'+Com_inputs[i].name+'，数量：'+Com_inputs[i].count+
        Com_inputs[i].unit+'，单价：'+Com_inputs[i].price+'(元)'+'，小计：'+per_Sum[i].toFixed(2)+'(元)\n';
  }
